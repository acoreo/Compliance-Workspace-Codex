...
class TestListModels(unittest.TestCase):
    def test_returns_model_names_without_tag(self):
        """list_models strips the :latest tag and returns bare names."""
        payload = json.dumps(
...
class TestAssertHealthy(unittest.TestCase):
    ...
    def test_raises_when_model_not_registered(self):
        ...
        with patch.object(backend, "health_check", return_value=True):
            with patch.object(backend, "list_models", return_value=["nemomix-local", "qwen2.5"]):
                with self.assertRaises(RuntimeError) as ctx:
                    backend.assert_healthy()
